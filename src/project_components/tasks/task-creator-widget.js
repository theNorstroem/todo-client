...
class TaskCreatorWidget extends FBP(LitElement) {
  ...
  static get styles() {
    // language=CSS
    return (
      Theme.getThemeForComponent('TaskCreatorWidget') ||
      css`
        :host {
          display: block;
  ...
  render() {
    ...
    return html`
      <furo-card
        header-text="Add a task"
        secondary-text="You can edit the task in detail later."
        ƒ-start-activity="--createBtnClicked"
        ƒ-stop-activity="--taskCreated"
      >
        <furo-form>
      ...
      <furo-entity-agent
        service="Tasks"
        ƒ-bind-request-data="--taskDO"
        ƒ-hts-in="--TasksHTS"
        ƒ-create="--createBtnClicked"
        @-create-success="--taskCreated(emptyObject)"
      ></furo-entity-agent>
      <furo-deep-link
        service="Tasks"
        ƒ-trigger="--FBPready"
